...
func registerAPIRoutes(r *gin.RouterGroup, h *RuleValueHandler, middleware ...gin.HandlerFunc) {
	// Apply middleware to the group if provided
	tx := r.Group("rule-value", middleware...)
	{
		tx.GET("/list", h.List)
...
func SetupRoutes(rg *gin.RouterGroup, h *RuleValueHandler, middleware ...gin.HandlerFunc) {
